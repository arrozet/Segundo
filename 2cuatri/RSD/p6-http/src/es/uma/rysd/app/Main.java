...
// - The correct one is inserted randomly
	...
	// Múltiple elección de Starships. Se debe elegir en función de la que se cree que es más cara.
	public static void priceyStarship(SWClient sw) {
		// En la API solo están los recursos 2, 3, 5, 9, 10, 11, 12, 13, 15, 17. En el count inicial pone que hay 36,
		// cuando en realidad hay solo 10 recursos disponibles.

		// De ahí que se den tantos errores 404 al intentar buscar recursos (por eso los he comentado,
		// para que no se haga pesado a quien use el "juego")

		// Obtener el número de naves espaciales
		int max_starship = sw.countResources("starships");

		if (max_starship == 0) {
			System.out.println("No se encontraron naves espaciales.");
			return;
		}

		System.out.println("Generando nueva pregunta...");

		// Lista para almacenar las naves espaciales seleccionadas
		List<SpaceShip> starships = new ArrayList<>();
		List<Integer> used = new ArrayList<>();

		// Seleccionar 2 naves espaciales con un costo conocido
		int contador = 0;
		while (contador < 2) {
			Integer p = getRandomResource(max_starship, used);
			SpaceShip starship = sw.getSpaceShip(sw.buildResourceUrl("starships", p));
			if (starship == null || starship.cost_in_credits.equals("unknown") || starship.cost_in_credits.equals("0")) {
				//System.out.println("	Hubo un error al encontrar el recurso " + p);
			} else {
		...
		Integer n = null;
		do {
			System.out.println("¿Cuál es la nave más cara?");
			for (int i = 0; i < 2; i++) {
				System.out.println(" [" + i + "] " + starships.get(i).name + " (" + starships.get(i).model + ")");
			}
			try {
				n = Integer.parseInt(sc.nextLine());
			} catch (NumberFormatException ex) {
		...
		// Encontrar la nave más cara
		SpaceShip mostExpensive = starships.get(0);
		SpaceShip otherShip = starships.get(0);
		for (SpaceShip ship : starships) {
			if (Double.parseDouble(ship.cost_in_credits) > Double.parseDouble(mostExpensive.cost_in_credits)) {
				mostExpensive = ship;
			}
			else{
				otherShip = ship;
		...
		for (SpaceShip ship : starships) {
			System.out.println(ship.name + " cuesta " + ship.cost_in_credits + " créditos");
		}
		// Hallo los precios de cada una para mostrar la diferencia
		double mostExpensivePrice = Double.parseDouble(mostExpensive.cost_in_credits);
		double otherPrice = Double.parseDouble(otherShip.cost_in_credits);
		System.out.print("\nLa nave más cara es " + mostExpensive.name + ", ");
		// Uso %.0f para que no salga en notación científica
		System.out.printf("costando %.0f créditos más que " + otherShip.name + ".\n\n",
				(mostExpensivePrice - otherPrice));

		// Resultados
		if (starships.get(n).equals(mostExpensive)) {
			System.out.println("Enhorabuena!!! " + success[rand.nextInt(success.length)]);
		} else {
			System.out.println("Fallaste :( " + error[rand.nextInt(error.length)]);
		}
