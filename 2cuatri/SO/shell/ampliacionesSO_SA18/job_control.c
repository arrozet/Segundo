...
job * new_job(pid_t pid, const char * command, enum job_state state)
{
    ...
    aux->next = NULL;
    // aqui no guardo la memoria para args
    return aux;
}
...
/* inserta elemento en la cabeza de la lista */
//RESPAWNABLE - AMPLIACIÓN 1
// Hecho de esta forma para solo guardar los args cuando lo necesito, ahorrando algo de memoria
// Además, de otra forma se ejecutaba respawnable indefinidamente
void add_respawnable_job (job * list, job * item, char** args)
{
    job * aux = list->next;
    ...
    int i=0;
    while(args[i]!=NULL){
        item->args[i] = strdup(args[i]);    // esto ya hace el malloc de args[i]
        i++;
    }
...
int delete_job(job * list, job * item)
{
    ...
    // para asegurarme que borro correctamente toda la memoria
    if(item->state == RESPAWNABLE){
        for(int i=0; item->args[i]!=NULL; i++){
            free(item->args[i]);    // esto ya hace el malloc de args[i]
        }
        free(item->args);
    }
