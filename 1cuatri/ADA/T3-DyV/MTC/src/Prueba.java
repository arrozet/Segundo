...
public class Prueba {
	...
	private static Result subvectorSM_DyV(int[] a, int start, int end) {
		if(start == end) {
			...
			return new Result (Integer.MIN_VALUE, Integer.MIN_VALUE, Integer.MIN_VALUE);
		}
		else {
			int mid = (start+end)/2;
			Result izq = subvectorSM_DyV(a, start, mid-1);	// si le pones un -1 debería ir en teoría pero da stack overflow
