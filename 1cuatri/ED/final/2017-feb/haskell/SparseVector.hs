...
vector n x 
  | n < 0     = error "vector: n can't be negative"
...
simplify :: (Eq a) => Tree a -> Tree a -> Tree a    -- esto no está del todo bien
simplify (Unif x) (Unif y)
  | x == y      = Unif x
  | otherwise   = Node (Unif x) (Unif y)
{-
simplify (Node i1@(Unif x1) d1@(Unif y1)) (Node i2@(Unif x2) d2@(Unif y2))  
  | x1==y1 && y1==x2 && x2==y2        = Unif x1
  | otherwise                         = Node (simplify i1 d1) (simplify i2 d2) 
-}
simplify (Unif x) (Node i2 d2)      = Node (Unif x) (simplify i2 d2)
simplify (Node i1 d1) (Unif x)      = Node (simplify i1 d1) (Unif x)
simplify (Node i1 d1) (Node i2 d2)  = Node (simplify i1 d1) (simplify i2 d2) 
...
-- | Exercise d.
get :: Int -> Vector a -> a
get index v@(V n (Unif x))  
  | index < 0 || index > ((size v)-1)   = error "get: wrong index"
  | otherwise                           = x

-- en el caso de unif, hay que buscar
get index v@(V n (Node t1 t2))
  | index < 0 || index > ((size v)-1)       = error "get: wrong index"
  | index < half_size                       = get index v1                      -- arbol izq
  | index >= half_size                      = get (index - half_size) v2            -- arbol der, debo ajustar indice descartando mitad de arbol
                                                                                    -- no se debe dividir pq hay conflictos despues
    where
      v1 = V half_size t1
      v2 = V half_size t2
      half_size = (size v) `div` 2 
...
set index x v@(V n (Unif y)) 
  | index < 0 || index > ((size v)-1)   = error "set: wrong index"
  | otherwise                           = V n (setT (size v) index x (Unif y))


set index x v@(V n (Node t1 t2))
  | index < 0 || index > ((size v)-1)         = error "set: wrong index"
  | index < half_size                         = V n (simplify (setT half_size index x t1) t2)                     -- arbol izq
  | index >= half_size                        = V n (simplify t1 (setT half_size (index - half_size) x t2))       -- arbol der, debo ajustar indice descartando mitad de arbol
    where
      half_size = (size v) `div` 2 

setT :: (Eq a) => Int -> Int -> a -> Tree a -> Tree a
setT sz index x t@(Unif y)
  | sz == 1             = (Unif x)
  | otherwise           = setT sz index x (Node (Unif y) (Unif y))

setT sz index x (Node t1 t2) 
  | index < new_sz      = simplify (setT new_sz index x t1) t2                     
  | index >= new_sz     = simplify t1 (setT new_sz (index-new_sz) x t2) 
    where
      new_sz = sz `div` 2
...
mapVector f v@(V tam t) = (V tam (mapTree f t))
  where
    mapTree f (Unif x)      = Unif (f x)
    mapTree f (Node t1 t2)  = simplify (mapTree f t1) (mapTree f t2)
