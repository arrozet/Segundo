...
size :: Trie a -> Int
size Empty        = 0
size t@(Node v d) = sizeValue v + aux (D.values d)
  where
    aux []      = 0
    aux (x:ts)  = size x + aux ts
...
childOf c t@(Node v d)
  | not (D.isDefinedAt c d) = Empty
  | otherwise               = toTrie (D.valueOf c d) 
...
search s t@(Node v d)
  | null s      = v
  | otherwise   = search (tail s) (childOf (head s) t)

-- | = Exercise h - update
update :: Trie a -> Char -> Trie a -> Trie a
update Empty c child          = Node Nothing (D.insert c child D.empty)
update t@(Node v d) c child   = Node v (D.insert c child d)

-- | = Exercise i - insert
insert :: String -> a -> Trie a -> Trie a
insert [] v Empty               = Node (Just v) D.empty
insert [] v t@(Node v1 d)       = Node (Just v) d
insert (c:cs) v t                = update t c hijo
  where
    hijo = insert cs v (childOf c t)
