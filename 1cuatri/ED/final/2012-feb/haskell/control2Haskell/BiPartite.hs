...
aux g dict stack
   ...
   | not (colored v)    = aux g (D.insert v color dict) (pushAll stack2 [(x, (nextColor color)) | x <- successors g v, not (colored x)])
   | otherwise          = if (D.valueOf' v dict) /= color then Nothing else aux g dict stack2
-- 
