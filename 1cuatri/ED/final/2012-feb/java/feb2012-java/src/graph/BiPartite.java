	...
	public BiPartite(Graph<V> graph) {
		...
		while (!stack.isEmpty() && isBiColored) {
			...
			if(dict.valueOf(vColor.first()) == null){
				// se asocia con su color
				dict.insert(vColor.first(), vColor.second());
				// los sucesores de v no visitados se apilan en el stack con su color correspondiente
				for(V suc : graph.successors(vColor.first())){
					if(dict.valueOf(suc)==null){
						stack.push(new Pair<>(suc, nextColor(vColor.second())));
					}
				}
			}
			else{
				if(dict.valueOf(vColor.first()) == vColor.second()){
					// no hago nada, everything its ok
				}
				else{
					isBiColored = false;
				}
